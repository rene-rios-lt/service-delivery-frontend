...
public class SignalRRequesterHubServiceTests
{
        ...
        Func<int, bool> connectSucceedsOnAttempt, Action<int> recordAttempt)
    {
        ...
        return new SignalRRequesterHubService(
            httpClient, _tokenStore.Object, NullLogger<SignalRRequesterHubService>.Instance,
            NullLoggerFactory.Instance, connect, noDelay,
        ...
        HubConnectionState state, Action recordConnectAttempt)
    {
        ...
        return new SignalRRequesterHubService(
            httpClient, _tokenStore.Object, NullLogger<SignalRRequesterHubService>.Instance,
            NullLoggerFactory.Instance, connect, noDelay, () => state);
    }
    ...
    [Fact]
    public void GivenAnInjectedLoggerFactory_WhenRequesterHubServiceIsConstructed_ThenTheHubConnectionLoggingIsRoutedToTheFactory()
    {
        // Arrange — QUAL-012: a client-side SignalR transport failure on the RequesterHub was invisible because
        // the HubConnection was built with no logger routing (its internal transport/dispatch logs went
        // nowhere). The service must now route that logging into the host's ILoggerFactory (mirroring the
        // FE-003 VehiclePositionHub fix). The HubConnection resolves ILoggerFactory when the builder constructs
        // it, so a factory routed via ConfigureLogging is asked for at least one logger during construction; a
        // spy factory that is NEVER asked proves the routing is absent. NullLoggerFactory would silently pass
        ...
        _ = new SignalRRequesterHubService(
            new HttpClient { BaseAddress = new Uri("http://localhost:5180") },
            _tokenStore.Object,
            NullLogger<SignalRRequesterHubService>.Instance,
