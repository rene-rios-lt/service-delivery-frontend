...
/// register a callback for the <c>JobOfferReceived</c> event (FE-020/AC-3, AC-5), register a
/// callback for the <c>RedirectReceived</c> event (FE-011/AC-6) so the active-job view can update its
/// destination in-place, and register a callback for the <c>JobOfferExpired</c> event (BUG-037) so
/// the offer screen dismisses the instant the server expires the offer instead of waiting for the
/// local countdown. Connection lifecycle and transport details live in the host-shared
...
public interface IRepHubService
{
    ...
    void OnJobOfferExpired(Func<JobOfferExpiredPayload, Task> handler);
}
